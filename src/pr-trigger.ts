...
export async function httpTrigger(context: Context, req: HttpRequest) {
    ...
    eventHandler.on(eventNames as unknown as typeof eventNames[number], handler);
    ...
    async function handler(event: EmitterWebhookEvent<typeof eventNames[number]>) {
        ...
        const pr: { number: number, title?: string } | undefined = prFromEvent();
        ...
        function prFromEvent() {
            switch (event.name) {
                case "check_suite": {
                    return event.payload.check_suite.pull_requests[0];
                }
                case "issue_comment":
                    return event.payload.issue;
                case "project_card": {
                    // Parse project_card.content_url according to
